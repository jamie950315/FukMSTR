...
import run_btcusdc_v150_funding_persistence_overlay as v150


ROOT = Path(__file__).resolve().parents[1]
OUT_DIR = ROOT / "runs" / "research_v151_range_alignment_overlay"
REPORT_PATH = ROOT / "reports" / "RESEARCH_V151_BTCUSDC_RANGE_ALIGNMENT_OVERLAY.md"
V150_ACCOUNT_PATH = ROOT / "runs" / "research_v150_funding_persistence_overlay" / "v150_selected_account_path.csv"
...
RANGE_ALIGNMENT_FEATURES = ("range_align_1440",)


@dataclass(frozen=True)
class RangeAlignmentSpec:
    name: str
    feature: str
    operator: str
    quantile: float
...
def _candidate_specs(frame: pd.DataFrame, selector_mask: pd.Series) -> list[RangeAlignmentSpec]:
    specs: list[RangeAlignmentSpec] = []
    for feature in RANGE_ALIGNMENT_FEATURES:
        if feature not in frame.columns:
            continue
        values = pd.to_numeric(frame.loc[selector_mask, feature], errors="coerce").dropna()
        if values.nunique() < 8:
            continue
        for quantile, threshold in values.quantile([0.5, 0.67, 0.75, 0.8, 0.85, 0.9]).drop_duplicates().items():
...
def _apply_overlay(frame: pd.DataFrame, spec: RangeAlignmentSpec) -> pd.DataFrame:
    out = frame.copy()
    ...
    out["v151_multiplier"] = multiplier
    out["v151_account_return_pct"] = out["v150_account_return_pct"] * multiplier
    out["v151_account_pnl_bps"] = out["v150_account_pnl_bps"] * multiplier
    return out
...
def _evaluate_candidate(
    ...
    *,
    ...
    changed = candidate_path["v151_multiplier"] != 1.0
    row: dict[str, object] = {
        "candidate": spec.name,
...
def _passes_v151_gate(candidate: dict[str, object], baseline: dict[str, dict[str, object]]) -> bool:
    if not candidate:
        return False
    return (
...
def _range_context_metrics(frame: pd.DataFrame) -> pd.DataFrame:
    out = frame.copy()
    out["range_align_bucket"] = pd.qcut(
        pd.to_numeric(out["range_align_1440"], errors="coerce"),
        q=5,
        duplicates="drop",
    )
    out["win"] = pd.to_numeric(out["v150_account_return_pct"], errors="coerce") > 0.0
    grouped = (
        out.groupby("range_align_bucket", observed=False)
        .agg(
            trade_count=("v150_account_return_pct", "size"),
            account_return_pct=("v150_account_return_pct", "sum"),
            win_rate=("win", "mean"),
    ...
    grouped["range_align_bucket"] = grouped["range_align_bucket"].astype(str)
...
def _write_report(
    ...
    lines = [
        ...
        f"- Promote to next model: `{decision['promote_to_v152']}`",
...
def run() -> dict[str, object]:
    ...
    if not V150_ACCOUNT_PATH.exists():
        v150.run()
    frame = pd.read_csv(V150_ACCOUNT_PATH)
    frame["timestamp"] = _to_utc(frame["timestamp"])
    for column in ("v150_account_return_pct", "v150_account_pnl_bps"):
    ...
    candidates.to_csv(OUT_DIR / "v151_range_alignment_candidates.csv", index=False)
    selected = _select_best_candidate(candidates)
    passed = _passes_v151_gate(selected, baseline)
    decision = {
        "status": "range_alignment_overlay_passed" if passed else "range_alignment_overlay_not_promoted",
        "promote_to_v152": bool(passed),
        "message": (
            "Range alignment improved V150 without worsening holdout/full risk gates."
            if passed
            else "Range alignment did not clear the promotion gate."
        ),
    }
    payload = {
        "config": {
            "base": "v150_funding_persistence_overlay",
    ...
    if selected:
        ...
        if selected_spec is not None:
            ...
            selected_path.to_csv(OUT_DIR / "v151_selected_account_path.csv", index=False)
            selected_monthly = (
                selected_path.assign(month=selected_path["timestamp"].dt.strftime("%Y-%m"))
                .groupby("month", sort=True)["v151_account_return_pct"]
                .sum()
                .reset_index()
                .rename(columns={"v151_account_return_pct": "account_return_pct"})
            )
    context_table = _range_context_metrics(frame)
    context_table.to_csv(OUT_DIR / "v151_range_context_metrics.csv", index=False)
    (OUT_DIR / "v151_range_alignment_summary.json").write_text(
