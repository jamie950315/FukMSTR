...
import run_btcusdc_v188_drought_trend_emotion_stepup as v188


ROOT = Path(__file__).resolve().parents[1]
OUT_DIR = ROOT / "runs" / "research_v189_rescue_mid_range_extreme_stepup"
REPORT_PATH = ROOT / "reports" / "RESEARCH_V189_BTCUSDC_RESCUE_MID_RANGE_EXTREME_STEPUP.md"
V188_ACCOUNT_PATH = ROOT / "runs" / "research_v188_drought_trend_emotion_stepup" / "v188_selected_account_path.csv"
...
class RescueMidRangeExtremeStepupPolicy:
    policy: str
    min_range_extreme_120: float | None = None
    target_indicator_key: str = "rescue_mid_0p62_0p66"
    target_side: str = "long"
    target_leg: str = "rescue"
    require_v188_action: str = "unchanged"
    stepup_multiplier: float = 1.0
    stepup_action: str = "rescue_mid_range_extreme_stepup"


POLICIES = (
    RescueMidRangeExtremeStepupPolicy(
        "v188_baseline_no_rescue_mid_range_extreme_stepup",
        stepup_multiplier=1.0,
        stepup_action="unchanged",
    ),
    RescueMidRangeExtremeStepupPolicy(
        "v189_rescue_mid_range_extreme120_ge0p807460_stepup1p25",
        min_range_extreme_120=0.807460,
        stepup_multiplier=1.25,
    ),
    RescueMidRangeExtremeStepupPolicy(
        "v189_rescue_mid_range_extreme120_ge0p807460_stepup1p15",
        min_range_extreme_120=0.807460,
...
def _stepup_mask(frame: pd.DataFrame, policy: RescueMidRangeExtremeStepupPolicy) -> pd.Series:
    ...
    if policy.min_range_extreme_120 is not None:
        range_extreme_120 = pd.to_numeric(frame.get("range_extreme_120", pd.Series(index=frame.index)), errors="coerce")
        mask &= range_extreme_120.ge(float(policy.min_range_extreme_120))
    return mask.fillna(False)


def _apply_rescue_mid_range_extreme_stepup_policy(
    trades: pd.DataFrame,
    policy: RescueMidRangeExtremeStepupPolicy,
) -> pd.DataFrame:
    out = trades.copy()
    ...
    base_return = pd.to_numeric(out["v188_account_return_pct"], errors="coerce").fillna(0.0)
    base_pnl = pd.to_numeric(out["v188_account_pnl_bps"], errors="coerce").fillna(0.0)
    out["v189_policy"] = policy.policy
    out["v189_state_multiplier"] = multiplier
    out["v189_state_action"] = action
    out["v189_account_return_pct"] = base_return * multiplier
    out["v189_account_pnl_bps"] = base_pnl * multiplier
    return out
...
def _month_concentration(path: pd.DataFrame) -> tuple[int, float]:
    work = path.loc[pd.to_numeric(path["v189_state_multiplier"], errors="coerce").fillna(1.0).gt(1.0)].copy()
    if work.empty:
        return 0, 0.0
    work["timestamp"] = _to_utc(work["timestamp"])
...
def _single_trade_delta_share(path: pd.DataFrame) -> float:
    multiplier = pd.to_numeric(path["v189_state_multiplier"], errors="coerce").fillna(1.0)
    stepped = multiplier.gt(1.0)
    if not stepped.any():
        return 0.0
    candidate_return = pd.to_numeric(path["v189_account_return_pct"], errors="coerce").fillna(0.0)
    if "v188_account_return_pct" in path.columns:
        base_return = pd.to_numeric(path["v188_account_return_pct"], errors="coerce").fillna(0.0)
    else:
...
def _policy_metrics(policy: str, path: pd.DataFrame, *, baseline_months: pd.Index) -> dict[str, object]:
    ...
    returns = pd.to_numeric(ordered["v189_account_return_pct"], errors="coerce").fillna(0.0)
    ...
    holdout_monthly = monthly[monthly.index >= SELECTOR_END.strftime("%Y-%m")]
    stepped = pd.to_numeric(ordered["v189_state_multiplier"], errors="coerce").fillna(1.0).gt(1.0)
...
def _compare_policies(policy_paths: dict[str, pd.DataFrame], baseline_months: pd.Index) -> pd.DataFrame:
    ...
    baseline = out.loc[out["policy"].eq("v188_baseline_no_rescue_mid_range_extreme_stepup")].iloc[0]
    ...
    out["range_extreme_stepup_passed"] = (
    ...
    out["range_extreme_stepup_score"] = (
    ...
    out["range_extreme_stepup_passed"] = out["range_extreme_stepup_passed"].map(bool).astype(object)
    return out.sort_values(
        ["range_extreme_stepup_passed", "range_extreme_stepup_score"], ascending=[False, False]
    ).reset_index(drop=True)


def _select_policy(comparison: pd.DataFrame) -> str:
    candidates = comparison.loc[
        comparison["range_extreme_stepup_passed"].astype(bool)
        & ~comparison["policy"].eq("v188_baseline_no_rescue_mid_range_extreme_stepup")
    ].copy()
    if candidates.empty:
        return "v188_baseline_no_rescue_mid_range_extreme_stepup"
...
def _monthly_path(path: pd.DataFrame) -> pd.DataFrame:
    ...
    work["baseline_return"] = pd.to_numeric(work["v188_account_return_pct"], errors="coerce").fillna(0.0)
    work["candidate_return"] = pd.to_numeric(work["v189_account_return_pct"], errors="coerce").fillna(0.0)
    work["stepped_up"] = pd.to_numeric(work["v189_state_multiplier"], errors="coerce").fillna(1.0).gt(1.0)
    return (
...
def _action_profile(path: pd.DataFrame) -> pd.DataFrame:
    ...
    work["baseline_return"] = pd.to_numeric(work["v188_account_return_pct"], errors="coerce").fillna(0.0)
    work["candidate_return"] = pd.to_numeric(work["v189_account_return_pct"], errors="coerce").fillna(0.0)
    work["candidate_pnl"] = pd.to_numeric(work["v189_account_pnl_bps"], errors="coerce").fillna(0.0)
    return (
        work.groupby(["v189_state_action", "v188_state_action", "indicator_key", "side", "leg"], dropna=False)
        .agg(
            trade_count=("candidate_return", "size"),
            baseline_return_pct=("baseline_return", "sum"),
            candidate_return_pct=("candidate_return", "sum"),
            win_rate_pct=("candidate_pnl", lambda s: float((s > 0.0).mean() * 100.0) if len(s) else 0.0),
            avg_multiplier=("v189_state_multiplier", "mean"),
            avg_range_extreme_120=("range_extreme_120", "mean"),
            avg_range_align_120=("range_align_120", "mean"),
            avg_prior_range_pos_120=("prior_range_pos_120", "mean"),
...
def _iteration_metrics_table(comparison: pd.DataFrame, *, selected_policy: str) -> list[dict[str, object]]:
    baseline = comparison.loc[comparison["policy"].eq("v188_baseline_no_rescue_mid_range_extreme_stepup")].iloc[0]
    selected = comparison.loc[comparison["policy"].eq(selected_policy)].iloc[0]
    rows = []
    for version, row, improvement in (
        ("V188", baseline, None),
        ("V189", selected, float(selected["return_delta_pct"])),
    ):
        rows.append(
            {
                "version": version,
...
def _payload_for_comparison(comparison: pd.DataFrame, *, selected_policy: str) -> dict[str, object]:
    ...
    selected_is_baseline = selected_policy == "v188_baseline_no_rescue_mid_range_extreme_stepup"
    return {
        "config": {
            "base": "v188_selected_account_path",
        ...
        "decision": {
            "status": (
                "rescue_mid_range_extreme_stepup_no_candidate"
                if selected_is_baseline
                else "rescue_mid_range_extreme_stepup_candidate_ready"
            ),
            "promote_to_live": False,
            "selected_policy": selected_policy,
            "selected_range_extreme_stepup_passed": bool(sel.get("range_extreme_stepup_passed", False)),
            ...
            "message": (
                "V189 adds an independent rescue-mid range-extreme size step-up and deliberately avoids "
                "another layer inside the narrow V188 drought trend emotion bucket."
            ),
        },
...
def _write_report(
    ...
    lines = [
        ...
        f"- Range-extreme step-up passed: `{decision['selected_range_extreme_stepup_passed']}`",
        f"- Return delta vs V188: `{decision['selected_return_delta_pct']}` pct",
        f"- Return improvement rate vs V188: `{decision['selected_return_improvement_rate']}`",
        f"- Drawdown improvement vs V188: `{decision['selected_drawdown_improvement_pct']}` pct",
        f"- Holdout return delta vs V188: `{decision['selected_holdout_return_delta_pct']}` pct",
        f"- Holdout drawdown improvement vs V188: `{decision['selected_holdout_drawdown_improvement_pct']}` pct",
...
def run() -> dict[str, object]:
    ...
    if not V188_ACCOUNT_PATH.exists():
        v188.run()
    trades = pd.read_csv(V188_ACCOUNT_PATH)
    trades["timestamp"] = _to_utc(trades["timestamp"])
    baseline_months = _baseline_months(trades)
    policy_paths = {
        policy.policy: _apply_rescue_mid_range_extreme_stepup_policy(trades, policy) for policy in POLICIES
    }
    ...
    comparison.to_csv(OUT_DIR / "v189_policy_comparison.csv", index=False)
    selected_path.to_csv(OUT_DIR / "v189_selected_account_path.csv", index=False)
    monthly.to_csv(OUT_DIR / "v189_selected_monthly_path.csv", index=False)
    profile.to_csv(OUT_DIR / "v189_selected_action_profile.csv", index=False)
    (OUT_DIR / "v189_rescue_mid_range_extreme_stepup_summary.json").write_text(
