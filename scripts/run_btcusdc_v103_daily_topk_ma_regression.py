...
OUT_DIR = ROOT / "runs" / "research_v103_btcusdc_daily_topk_ma_regression"
REPORT_PATH = ROOT / "reports" / "RESEARCH_V103_BTCUSDC_DAILY_TOPK_MA_REGRESSION_RESULTS.md"

HORIZONS = (15, 30, 60)
MIN_EDGE_FLOORS_BPS = (0.0, 4.0, 8.5, 10.0, 12.5)
DAILY_TOP_KS = (1, 2, 3)
...
def _non_overlapping_daily_indices(day_frame: pd.DataFrame, *, daily_top_k: int, horizon_minutes: int) -> list[int]:
    ranked = day_frame.assign(_abs_pred=day_frame["predicted_return_bps"].abs()).sort_values(
        ["_abs_pred", "timestamp"],
...
def _daily_topk_prediction_ledger(
    ...
    *,
    ...
    frame["future_return_bps"] = pd.to_numeric(frame["future_return_bps"], errors="coerce")
    frame["predicted_return_bps"] = pd.to_numeric(frame["predicted_return_bps"], errors="coerce")
    frame = frame.dropna(subset=["timestamp", "future_return_bps", "predicted_return_bps"]).copy()
    pred = frame["predicted_return_bps"]
    frame = frame.loc[pred.abs() >= float(min_edge_bps)].copy()
    frame = frame.loc[pred != 0.0].copy()
    if frame.empty:
        return pd.DataFrame(
            columns=[
                "timestamp",
    ...
    for _, day_frame in frame.groupby(frame["timestamp"].dt.normalize(), sort=True):
        keep_idx.extend(
            _non_overlapping_daily_indices(
                day_frame,
    ...
    signal = np.where(predicted > 0.0, 1, -1).astype(int)
    gross = future * signal
    ledger = pd.DataFrame(
        {
...
def _passes_daily_topk_gate(row: dict[str, object]) -> bool:
    return (
...
def _evaluate_horizon(bars: pd.DataFrame, *, horizon: int) -> tuple[list[dict[str, object]], dict[str, pd.DataFrame], dict[str, object]]:
    ...
    for min_edge in MIN_EDGE_FLOORS_BPS:
        for daily_top_k in DAILY_TOP_KS:
            selector_ledger = _daily_topk_prediction_ledger(
                selector_pred,
                daily_top_k=int(daily_top_k),
            ...
            holdout_ledger = _daily_topk_prediction_ledger(
                holdout_pred,
                daily_top_k=int(daily_top_k),
                min_edge_bps=float(min_edge),
                horizon_minutes=int(horizon),
            ...
            row = {
                "policy_id": f"hgb_ma_daily_top{int(daily_top_k)}_h{int(horizon)}_floor{float(min_edge):g}",
                "horizon_minutes": int(horizon),
                "daily_top_k": int(daily_top_k),
                "min_edge_bps": float(min_edge),
            ...
            row["passed_daily_topk_gate"] = bool(_passes_daily_topk_gate(row))
            rows.append(row)
            if bool(row["passed_daily_topk_gate"]):
...
def _scan(bars: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], list[dict[str, object]]]:
    ...
    for horizon in HORIZONS:
        ...
        metas.append(meta)
        print(f"evaluated daily top-k MA regression horizon {horizon} with {len(horizon_rows)} policies", flush=True)
    candidates = pd.DataFrame(rows)
    if not candidates.empty:
        candidates = candidates.sort_values(
            [
                "passed_daily_topk_gate",
...
def _write_report(payload: dict[str, object], candidates: pd.DataFrame, passed: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- Passing daily top-k candidates: `{payload['decision']['passing_candidate_count']}`",
        f"- Selected candidate: `{payload['decision']['selected_policy']}`",
        f"- Fee: `{FEE_BPS}` bps",
        f"- Daily top-k values: `{list(DAILY_TOP_KS)}`",
        f"- Minimum edge floors: `{list(MIN_EDGE_FLOORS_BPS)}` bps",
        "",
        "## Top Candidates",
        "",
        ...
        "## Passing Candidates",
        "",
        passed[report_cols].to_csv(index=False).strip() if not passed.empty else "No candidate passed the daily top-k gate.",
        "",
        "## Interpretation",
        "",
...
def run() -> dict[str, object]:
    ...
    passed = candidates.loc[candidates["passed_daily_topk_gate"]].copy() if not candidates.empty else pd.DataFrame()

    candidates_path = OUT_DIR / "v103_daily_topk_candidates.csv"
    passed_path = OUT_DIR / "v103_daily_topk_passed_candidates.csv"
    candidates.to_csv(candidates_path, index=False)
    passed.to_csv(passed_path, index=False)
    for policy_id, ledger in ledgers.items():
        ledger.to_csv(OUT_DIR / f"v103_{policy_id}_trade_ledger.csv", index=False)

    selected = str(passed.iloc[0]["policy_id"]) if not passed.empty else None
    payload = {
        "version": "v103_btcusdc_daily_topk_ma_regression",
        "scan": {
            "candidate_count": int(len(candidates)),
            "horizons": list(HORIZONS),
            "daily_top_ks": list(DAILY_TOP_KS),
            "min_edge_floors_bps": list(MIN_EDGE_FLOORS_BPS),
            "fee_bps": float(FEE_BPS),
        ...
        "decision": {
            ...
            "failed_reason": None if selected is not None else "no candidate passed the 8.5 bps daily top-k MA high-frequency gate",
        },
        "outputs": {
    ...
    (OUT_DIR / "v103_summary.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
