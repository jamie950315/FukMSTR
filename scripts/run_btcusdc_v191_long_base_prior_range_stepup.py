...
import run_btcusdc_v190_short_base_prior_rally_stepup as v190


ROOT = Path(__file__).resolve().parents[1]
OUT_DIR = ROOT / "runs" / "research_v191_long_base_prior_range_stepup"
REPORT_PATH = ROOT / "reports" / "RESEARCH_V191_BTCUSDC_LONG_BASE_PRIOR_RANGE_STEPUP.md"
V190_ACCOUNT_PATH = ROOT / "runs" / "research_v190_short_base_prior_rally_stepup" / "v190_selected_account_path.csv"
...
class LongBasePriorRangeStepupPolicy:
    policy: str
    min_prior_range_pos_1440: float | None = None
    ...
    stepup_action: str = "long_base_prior_range_stepup"


POLICIES = (
    LongBasePriorRangeStepupPolicy(
        "v190_baseline_no_long_base_prior_range_stepup",
        stepup_multiplier=1.0,
        stepup_action="unchanged",
    ),
    LongBasePriorRangeStepupPolicy(
        "v191_long_base_priorrangepos1440_ge0p326636_stepup1p25",
        min_prior_range_pos_1440=0.326636,
        stepup_multiplier=1.25,
    ),
    LongBasePriorRangeStepupPolicy(
        "v191_long_base_priorrangepos1440_ge0p326636_stepup1p15",
        min_prior_range_pos_1440=0.326636,
...
def _stepup_mask(frame: pd.DataFrame, policy: LongBasePriorRangeStepupPolicy) -> pd.Series:
    ...
    if policy.min_prior_range_pos_1440 is not None:
        prior_range = pd.to_numeric(frame.get("prior_range_pos_1440", pd.Series(index=frame.index)), errors="coerce")
        mask &= prior_range.ge(float(policy.min_prior_range_pos_1440))
    return mask.fillna(False)


def _apply_long_base_prior_range_stepup_policy(
    trades: pd.DataFrame,
    policy: LongBasePriorRangeStepupPolicy,
) -> pd.DataFrame:
    out = trades.copy()
    ...
    base_return = pd.to_numeric(out["v190_account_return_pct"], errors="coerce").fillna(0.0)
    base_pnl = pd.to_numeric(out["v190_account_pnl_bps"], errors="coerce").fillna(0.0)
    out["v191_policy"] = policy.policy
    out["v191_state_multiplier"] = multiplier
    out["v191_state_action"] = action
    out["v191_account_return_pct"] = base_return * multiplier
    out["v191_account_pnl_bps"] = base_pnl * multiplier
    return out
...
def _month_concentration(path: pd.DataFrame) -> tuple[int, float]:
    work = path.loc[pd.to_numeric(path["v191_state_multiplier"], errors="coerce").fillna(1.0).gt(1.0)].copy()
    if work.empty:
        return 0, 0.0
    work["timestamp"] = _to_utc(work["timestamp"])
...
def _single_trade_delta_share(path: pd.DataFrame) -> float:
    multiplier = pd.to_numeric(path["v191_state_multiplier"], errors="coerce").fillna(1.0)
    stepped = multiplier.gt(1.0)
    if not stepped.any():
        return 0.0
    candidate_return = pd.to_numeric(path["v191_account_return_pct"], errors="coerce").fillna(0.0)
    if "v190_account_return_pct" in path.columns:
        base_return = pd.to_numeric(path["v190_account_return_pct"], errors="coerce").fillna(0.0)
    else:
...
def _policy_metrics(policy: str, path: pd.DataFrame, *, baseline_months: pd.Index) -> dict[str, object]:
    ...
    returns = pd.to_numeric(ordered["v191_account_return_pct"], errors="coerce").fillna(0.0)
    ...
    holdout_monthly = monthly[monthly.index >= SELECTOR_END.strftime("%Y-%m")]
    stepped = pd.to_numeric(ordered["v191_state_multiplier"], errors="coerce").fillna(1.0).gt(1.0)
...
def _compare_policies(policy_paths: dict[str, pd.DataFrame], baseline_months: pd.Index) -> pd.DataFrame:
    ...
    baseline = out.loc[out["policy"].eq("v190_baseline_no_long_base_prior_range_stepup")].iloc[0]
    ...
    out["prior_range_stepup_passed"] = out["prior_range_stepup_passed"].map(bool).astype(object)
    return out.sort_values(["prior_range_stepup_passed", "prior_range_stepup_score"], ascending=[False, False]).reset_index(
        drop=True
    )
...
def _select_policy(comparison: pd.DataFrame) -> str:
    candidates = comparison.loc[
        comparison["prior_range_stepup_passed"].astype(bool)
        & ~comparison["policy"].eq("v190_baseline_no_long_base_prior_range_stepup")
    ].copy()
    if candidates.empty:
        return "v190_baseline_no_long_base_prior_range_stepup"
...
def _monthly_path(path: pd.DataFrame) -> pd.DataFrame:
    ...
    work["baseline_return"] = pd.to_numeric(work["v190_account_return_pct"], errors="coerce").fillna(0.0)
    work["candidate_return"] = pd.to_numeric(work["v191_account_return_pct"], errors="coerce").fillna(0.0)
    work["stepped_up"] = pd.to_numeric(work["v191_state_multiplier"], errors="coerce").fillna(1.0).gt(1.0)
    return (
...
def _action_profile(path: pd.DataFrame) -> pd.DataFrame:
    ...
    work["baseline_return"] = pd.to_numeric(work["v190_account_return_pct"], errors="coerce").fillna(0.0)
    work["candidate_return"] = pd.to_numeric(work["v191_account_return_pct"], errors="coerce").fillna(0.0)
    work["candidate_pnl"] = pd.to_numeric(work["v191_account_pnl_bps"], errors="coerce").fillna(0.0)
    return (
        work.groupby(["v191_state_action", "v190_state_action", "v189_state_action", "v188_state_action", "indicator_key", "side", "leg"], dropna=False)
        .agg(
            trade_count=("candidate_return", "size"),
            baseline_return_pct=("baseline_return", "sum"),
            candidate_return_pct=("candidate_return", "sum"),
            win_rate_pct=("candidate_pnl", lambda s: float((s > 0.0).mean() * 100.0) if len(s) else 0.0),
            avg_multiplier=("v191_state_multiplier", "mean"),
            avg_prior_range_pos_1440=("prior_range_pos_1440", "mean"),
            avg_prior_ret_1440_bps=("prior_ret_1440_bps", "mean"),
            avg_range_extreme_1440=("range_extreme_1440", "mean"),
...
def _iteration_metrics_table(comparison: pd.DataFrame, *, selected_policy: str) -> list[dict[str, object]]:
    baseline = comparison.loc[comparison["policy"].eq("v190_baseline_no_long_base_prior_range_stepup")].iloc[0]
    selected = comparison.loc[comparison["policy"].eq(selected_policy)].iloc[0]
    rows = []
    for version, row, improvement in (
        ("V190", baseline, None),
        ("V191", selected, float(selected["return_delta_pct"])),
    ):
        rows.append(
            {
                "version": version,
...
def _payload_for_comparison(comparison: pd.DataFrame, *, selected_policy: str) -> dict[str, object]:
    ...
    selected_is_baseline = selected_policy == "v190_baseline_no_long_base_prior_range_stepup"
    return {
        "config": {
            "base": "v190_selected_account_path",
        ...
        "decision": {
            "status": (
                "long_base_prior_range_stepup_no_candidate"
                if selected_is_baseline
                else "long_base_prior_range_stepup_candidate_ready"
            ),
            "promote_to_live": False,
            "selected_policy": selected_policy,
            "selected_prior_range_stepup_passed": bool(sel.get("prior_range_stepup_passed", False)),
            ...
            "message": (
                "V191 adds an independent long-base prior-range size step-up and avoids modifying rows already "
                "changed by V188, V189, or V190."
            ),
        },
...
def _write_report(
    ...
    lines = [
        ...
        f"- Prior-range step-up passed: `{decision['selected_prior_range_stepup_passed']}`",
        f"- Return delta vs V190: `{decision['selected_return_delta_pct']}` pct",
        f"- Return improvement rate vs V190: `{decision['selected_return_improvement_rate']}`",
        f"- Drawdown improvement vs V190: `{decision['selected_drawdown_improvement_pct']}` pct",
        f"- Holdout return delta vs V190: `{decision['selected_holdout_return_delta_pct']}` pct",
        f"- Holdout drawdown improvement vs V190: `{decision['selected_holdout_drawdown_improvement_pct']}` pct",
...
def run() -> dict[str, object]:
    ...
    if not V190_ACCOUNT_PATH.exists():
        v190.run()
    trades = pd.read_csv(V190_ACCOUNT_PATH)
    trades["timestamp"] = _to_utc(trades["timestamp"])
    baseline_months = _baseline_months(trades)
    policy_paths = {policy.policy: _apply_long_base_prior_range_stepup_policy(trades, policy) for policy in POLICIES}
    ...
    comparison.to_csv(OUT_DIR / "v191_policy_comparison.csv", index=False)
    selected_path.to_csv(OUT_DIR / "v191_selected_account_path.csv", index=False)
    monthly.to_csv(OUT_DIR / "v191_selected_monthly_path.csv", index=False)
    profile.to_csv(OUT_DIR / "v191_selected_action_profile.csv", index=False)
    (OUT_DIR / "v191_long_base_prior_range_stepup_summary.json").write_text(
