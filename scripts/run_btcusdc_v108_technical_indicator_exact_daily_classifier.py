...
OUT_DIR = ROOT / "runs" / "research_v108_btcusdc_technical_indicator_exact_daily_classifier"
REPORT_PATH = ROOT / "reports" / "RESEARCH_V108_BTCUSDC_TECHNICAL_INDICATOR_EXACT_DAILY_CLASSIFIER_RESULTS.md"
V106_SELECTED_PATH = v106.OUT_DIR / "v106_selector_locked_selected_candidate.csv"

RSI_WINDOWS = (7, 14, 28)
BOLLINGER_WINDOWS = (20, 60)
ATR_WINDOWS = (14, 30)
STOCHASTIC_WINDOWS = (14, 30)
...
TECHNICAL_FEATURE_COLUMNS = [
    *[f"rsi_{window}" for window in RSI_WINDOWS],
    ...
    "macd_signal_bps",
    "macd_hist_bps",
    *[f"bollinger_z_{window}" for window in BOLLINGER_WINDOWS],
    *[f"bollinger_width_{window}_bps" for window in BOLLINGER_WINDOWS],
    *[f"bollinger_upper_{window}_dist_bps" for window in BOLLINGER_WINDOWS],
    *[f"bollinger_lower_{window}_dist_bps" for window in BOLLINGER_WINDOWS],
    *[f"atr_{window}_bps" for window in ATR_WINDOWS],
    *[f"stoch_pos_{window}" for window in STOCHASTIC_WINDOWS],
]
...
def _add_technical_indicator_features(
    bars: pd.DataFrame,
    *,
    rsi_windows: tuple[int, ...] = RSI_WINDOWS,
    bollinger_windows: tuple[int, ...] = BOLLINGER_WINDOWS,
    atr_windows: tuple[int, ...] = ATR_WINDOWS,
    stochastic_windows: tuple[int, ...] = STOCHASTIC_WINDOWS,
) -> pd.DataFrame:
    ...
    prev_prior_close = prior_close.shift(1)

    prior_delta = prior_close.diff()
    gain = prior_delta.clip(lower=0.0)
    loss = (-prior_delta.clip(upper=0.0))
    ...
    ema_fast = prior_close.ewm(span=12, adjust=False, min_periods=12).mean()
    ema_slow = prior_close.ewm(span=26, adjust=False, min_periods=26).mean()
    macd_line = ema_fast - ema_slow
    macd_signal = macd_line.ewm(span=9, adjust=False, min_periods=9).mean()
    macd_hist = macd_line - macd_signal
    out["macd_line_bps"] = macd_line / open_px * 10000.0
    out["macd_signal_bps"] = macd_signal / open_px * 10000.0
    out["macd_hist_bps"] = macd_hist / open_px * 10000.0

    for window in bollinger_windows:
        window = int(window)
        mid = prior_close.rolling(window, min_periods=window).mean()
        std = prior_close.rolling(window, min_periods=window).std()
        upper = mid + 2.0 * std
        lower = mid - 2.0 * std
        out[f"bollinger_mid_{window}"] = mid
        out[f"bollinger_z_{window}"] = (open_px - mid) / std.replace(0.0, np.nan)
        out[f"bollinger_width_{window}_bps"] = (upper / lower - 1.0) * 10000.0
        out[f"bollinger_upper_{window}_dist_bps"] = (open_px / upper - 1.0) * 10000.0
        out[f"bollinger_lower_{window}_dist_bps"] = (open_px / lower - 1.0) * 10000.0

    true_range = pd.concat(
        [
            prior_high - prior_low,
            (prior_high - prev_prior_close).abs(),
            (prior_low - prev_prior_close).abs(),
        ],
        axis=1,
    ).max(axis=1)
    for window in atr_windows:
        ...
        out[f"atr_{window}_bps"] = atr / open_px * 10000.0
    ...
    for window in stochastic_windows:
        ...
        out[f"stoch_pos_{window}"] = (open_px - rolling_low) / (rolling_high - rolling_low).replace(0.0, np.nan)

    return out


def _technical_indicator_feature_frame(bars: pd.DataFrame, *, horizon_minutes: int) -> tuple[pd.DataFrame, list[str]]:
    sampled, feature_cols = v102._ma_feature_frame(bars, horizon_minutes=int(horizon_minutes))
    technical_frame = _add_technical_indicator_features(bars)[["timestamp", *TECHNICAL_FEATURE_COLUMNS]].copy()
    sampled = sampled.merge(technical_frame, on="timestamp", how="left")
...
def _evaluate_horizon(bars: pd.DataFrame, *, horizon: int) -> tuple[list[dict[str, object]], dict[str, pd.DataFrame], dict[str, object]]:
    data, feature_cols = _technical_indicator_feature_frame(bars, horizon_minutes=int(horizon))
    ...
    for probability_floor in PROBABILITY_FLOORS:
        for daily_top_k in DAILY_TOP_KS:
            ...
            row = {
                "policy_id": f"hgb_technical_exact_daily_top{int(daily_top_k)}_h{int(horizon)}_p{float(probability_floor):.6f}",
                "horizon_minutes": int(horizon),
                "daily_top_k": int(daily_top_k),
                "probability_floor": float(probability_floor),
    ...
    meta = {
        ...
        "holdout_rows": int(len(holdout)),
        "feature_count": int(len(feature_cols)),
        "technical_feature_count": int(len(TECHNICAL_FEATURE_COLUMNS)),
...
def _scan(bars: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], list[dict[str, object]]]:
    ...
    for horizon in HORIZONS:
        ...
        metas.append(meta)
        print(f"evaluated technical-indicator exact-daily classifier horizon {horizon} with {len(horizon_rows)} policies", flush=True)
...
def _comparison_against_v106(selected_row: pd.DataFrame) -> dict[str, object]:
    ...
    return {
        ...
        "v108_policy_id": str(current["policy_id"]),
...
def _write_report(
    ...
    if comparison.get("available"):
        comparison_lines = [
            ...
            f"- V108 policy: `{comparison['v108_policy_id']}`",
    ...
    lines = [
        ...
        f"- Technical features added: `{len(TECHNICAL_FEATURE_COLUMNS)}`",
        "",
        "## Selected Candidate",
        "",
...
def run() -> dict[str, object]:
    ...
    candidates_path = OUT_DIR / "v108_technical_indicator_candidates.csv"
    passed_path = OUT_DIR / "v108_technical_indicator_passed_candidates.csv"
    selected_path = OUT_DIR / "v108_selector_locked_selected_candidate.csv"
    ...
    for policy_id, ledger in ledgers.items():
        ledger.to_csv(OUT_DIR / f"v108_{policy_id}_trade_ledger.csv", index=False)

    payload = {
        "version": "v108_btcusdc_technical_indicator_exact_daily_classifier",
        "scan": {
            ...
            "technical_feature_columns": list(TECHNICAL_FEATURE_COLUMNS),
    ...
    (OUT_DIR / "v108_summary.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
