...
OUT_DIR = ROOT / "runs" / "research_v158_base_range_position_boost"
REPORT_PATH = ROOT / "reports" / "RESEARCH_V158_BTCUSDC_BASE_RANGE_POSITION_BOOST.md"
...
def _overlay_spec(frame: pd.DataFrame, selector_mask: pd.Series, *, min_unique: int = 8) -> BaseRangePositionSpec:
...
def _condition(frame: pd.DataFrame, spec: BaseRangePositionSpec) -> pd.Series:
    return _segment_mask(frame, spec.segment) & _compare(frame[spec.feature], spec.operator, spec.threshold)


def _apply_overlay(frame: pd.DataFrame, spec: BaseRangePositionSpec) -> pd.DataFrame:
    out = frame.copy()
    flag = _condition(out, spec).fillna(False)
    modifier = pd.Series(1.0, index=out.index)
    modifier.loc[flag] = spec.modifier
    out["v158_base_range_position_boost_flag"] = flag
    out["v158_modifier"] = modifier
    out["v158_account_return_pct"] = out["v156_account_return_pct"] * modifier
    out["v158_account_pnl_bps"] = out["v156_account_pnl_bps"] * modifier
    return out
...
def _candidate_metrics(
    ...
    *,
    spec: BaseRangePositionSpec,
    masks: dict[str, pd.Series],
    baseline_metrics: dict[str, dict[str, object]],
    baseline_months: dict[str, pd.Index],
) -> dict[str, object]:
    changed = candidate_path["v158_modifier"] != 1.0
    flag = candidate_path["v158_base_range_position_boost_flag"]
    row: dict[str, object] = {
        "candidate": "v158_base_range_position_boost",
...
def _passes_v158_gate(candidate: dict[str, object], baseline: dict[str, dict[str, object]]) -> bool:
    if not candidate:
        return False
    return (
...
def _context_metrics(frame: pd.DataFrame, candidate_path: pd.DataFrame) -> pd.DataFrame:
    out = frame.copy()
    out["v158_bucket"] = "unchanged"
    out.loc[candidate_path["v158_base_range_position_boost_flag"], "v158_bucket"] = "base_range_position_boost"
    out["win"] = pd.to_numeric(out["v156_account_return_pct"], errors="coerce") > 0.0
    grouped = (
        out.groupby("v158_bucket", observed=False)
        .agg(
            trade_count=("v156_account_return_pct", "size"),
            v156_account_return_pct=("v156_account_return_pct", "sum"),
            win_rate=("win", "mean"),
...
def _write_report(
    ...
    lines = [
        ...
        f"- Promote to next model: `{decision['promote_to_v159']}`",
        ...
        f"- Boost: `{SEGMENT}` trades where `{FEATURE} {OPERATOR} selector q{QUANTILE}` use `{MODIFIER}x` sizing on top of V156.",
...
def run() -> dict[str, object]:
    ...
    passed = _passes_v158_gate(selected, baseline)
    decision = {
        "status": "base_range_position_boost_passed" if passed else "base_range_position_boost_not_promoted",
        "promote_to_v159": bool(passed),
        "message": (
            "Base range-position boost improved V156 by at least 1% without worsening selector/full/holdout risk gates."
            if passed
            else "Base range-position boost did not clear the promotion gate."
        ),
    }
    payload = {
        "config": {
    ...
    selected_path.to_csv(OUT_DIR / "v158_selected_account_path.csv", index=False)
    pd.DataFrame([selected]).to_csv(OUT_DIR / "v158_base_range_position_boost_candidate.csv", index=False)
    selected_monthly = (
        selected_path.assign(month=selected_path["timestamp"].dt.strftime("%Y-%m"))
        .groupby("month", sort=True)["v158_account_return_pct"]
        .sum()
        .reset_index()
        .rename(columns={"v158_account_return_pct": "account_return_pct"})
    )
    selected_monthly.to_csv(OUT_DIR / "v158_monthly_account_return.csv", index=False)
    context_table = _context_metrics(frame, selected_path)
    context_table.to_csv(OUT_DIR / "v158_base_range_position_context_metrics.csv", index=False)
    (OUT_DIR / "v158_base_range_position_boost_summary.json").write_text(
