...
def _toy_predictions(rows: int = 20) -> pd.DataFrame:
    ts = pd.date_range("2020-01-01", periods=rows, freq="1s", tz="UTC")
    mid = 100.0 + 0.04 * pd.Series(range(rows), dtype=float)
    return pd.DataFrame(
        {
...
def test_run_profit_exit_lock_writes_summary(tmp_path: Path) -> None:
    base = tmp_path / "base"
    ...
    result = run_profit_exit_lock(
        ...
        shift_null_runs=1,
        gate=ProfitExitLockGate(min_oof_trades=1, min_folds_with_trades=1, max_family_p=1.0),
        clean=True,
    )
    assert result["aggregate"]["trades"] >= 1
    assert (out / "summary.json").exists()
    assert (out / "profit_exit_lock_oof_backtest.csv").exists()
    assert (out / "exit_lock_family_candidates.csv").exists()
